...
public class DoublingTest {
    ...
    public static double timeTrial_2(int[] n) {
    	Stopwatch timer = new Stopwatch();
    	IntQuick.sort(n);
    	return timer.elapsedTime();
    }
    
    public static double timeTrialNS(int[] n) {
    	Integer[] a = new Integer[n.length];
        for (int i = 0; i < n.length; i++) {
            a[i] = n[i];
        }
    ...
    public static double timeTrial_2NS(int[] n) {
    	Stopwatch timer = new Stopwatch();
    	IntQuick.sortNonShuffle(n);
    	return timer.elapsedTime();
    }
    ...
    public static void main(String[] args) { 
    	...
    	int[] asc = StdIn.readAllInts();
    	StdOut.println("done");
    	int[] des = StdIn.readAllInts();
    	StdOut.println("done");
    	int[] ran = StdIn.readAllInts();
    	StdOut.println("done");
    	...
    	StdOut.printf("ascending - descending - random \n %f - %f - %f\n",
    			timeTrial_2(asc), timeTrial_2(des), timeTrial_2(ran));
    	StdOut.printf("ascending - descending - random \n %f - %f - %f",
    			timeTrial_2NS(ascNS), timeTrial_2NS(desNS), timeTrial_2NS(ranNS));
    } 
}
